...
class _MyAppState extends State<MyApp> {
  ...
  void _toggleFavorite(String mealId) {
    final existingIndex =
        _favoriteMeald.indexWhere((meal) => meal.id == mealId);
    if (existingIndex >= 0) {
      setState(() {
        _favoriteMeald.removeAt(existingIndex);
      });
    } else {
      setState(() {
        _favoriteMeald.add(
  ...
  bool _isMealFavorite(String id) {
    return _favoriteMeald.any((meal) => meal.id == meal.id);
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: TabsScreen(_favoriteMeald),
      routes: {
